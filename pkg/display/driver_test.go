...
// AUTO-DESKTOP: auto prefers GLFW when it is installed, regardless of
// registration order. If GLFW is absent, another non-web driver remains a
// valid fallback.
	...
	for _, order := range [][]string{
		...
		t.Run(strings.Join(order, "+"), func(t *testing.T) {
			...
			if got := GetDriver("web"); got != want {
				t.Fatalf("GetDriver(web) with %v installed = %v, want the registered web driver", order)
			}
		})
	}
}
